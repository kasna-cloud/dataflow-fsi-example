...
from tfx_bsl.public.proto.model_spec_pb2 import InferenceSpecType, AIPlatformPredictionModelSpec
...
def run_windowed_inference(
	...
	def _parse_prediction_log(prediction_log):
		...
		for combined_name, tensor in response_output.items():
			...
			features_by_type[output_type][feature_name] = tf.make_ndarray(tensor).flatten()
		...
		for output_type, list_per_feature in features_by_type.items():
			for length in [len(values) for values in list_per_feature.values()]:
				if length != window_length:
					raise ValueError("Prediction output features do not have the same window length!")
			
			parsed_outputs[output_type] = [{f: vs[i] for f, vs in list_per_feature.items()} for i in range(window_length)]
	...
	return (input_elements
		...
		| "ParsePredictionLog" >> beam.MapTuple(lambda k, v: (k, _parse_prediction_log(v)))
