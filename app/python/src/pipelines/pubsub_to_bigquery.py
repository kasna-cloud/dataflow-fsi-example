...
from src.serialisers.pubsub import PubSubSerialiser, parse_pubsub_topic_or_subscription_str
...
def setup_parser(parser):
	parser.add_argument(
		...
		required=True,
		nargs="+",
		help=("String mapping of in the form of <label>::<input_pubsub>::<output_bigquery>, where "
...
def run_pipeline(
	...
	with beam.Pipeline(options=pipeline_options) as pipeline:
		for mapping in mappings:
			...
			input_topic, input_subscription = parse_pubsub_topic_or_subscription_str(input_ps)
			pubsub_serialiser = PubSubSerialiser(timestamp_key)
			(pipeline
				| f"Read from Pubsub ({label})" >> beam.io.ReadFromPubSub(
					topic=input_topic,
					subscription=input_subscription,
					timestamp_attribute=timestamp_key,
				)
				| f"Deserialise Json ({label})" >> beam.Map(pubsub_serialiser.to_json)
				| f"Write to BigQuery ({label})" >> beam.io.WriteToBigQuery(
					table=output_bq,
